...
검색은 별도 색인 서버 없이 **Postgres 안에서** 한다. OpenSearch 컨테이너가 한동안
떠 있었지만 읽는 코드가 한 줄도 없었고, 2026-09-08에 걷어냈다: 켜는 순간 DB와
동기를 맞춰야 하는 두 번째 저장소가 되고, 이 시스템에서 조용히 썩은 것들은 전부
"둘이 어긋나는데 아무도 안 보는" 종류였다. 전문검색은 `ledger_extracted_text`의
tsvector 색인이고, 그것으로 부족하다고 **실측되면** 같은 데이터베이스 안의
`pgvector`가 다음 단계다. postgres 이미지가 `pgvector/pgvector:pg17`인 이유다.
